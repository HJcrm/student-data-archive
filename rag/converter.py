...
class DataConverter:
    ...
    MAJOR_FIELD_KEYWORDS = {
        "경영/경제": ["경영", "경제", "금융", "회계", "무역", "국제통상", "세무", "재무"],
        "인문": ["국어국문", "영어영문", "사학", "철학", "문학", "언어", "한문"],
        "사회": ["정치", "외교", "행정", "사회학", "심리", "사회복지", "언론", "미디어", "법학"],
        "자연": ["수학", "물리", "화학", "생명", "생물", "지구과학", "천문"],
        "공학": ["컴퓨터", "전자", "전기", "기계", "화공", "건축", "토목", "산업공학", "소프트웨어"],
        "의약": ["의학", "의예", "치의", "한의", "약학", "간호"],
        "교육": ["교육", "사범"],
    ...
    def _infer_major_field(self, dept: Optional[str], theme: Optional[str]) -> Optional[str]:
        """계열 추론"""
        search_text = f"{dept or ''} {theme or ''}"

        for field, keywords in self.MAJOR_FIELD_KEYWORDS.items():
